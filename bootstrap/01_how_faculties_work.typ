...
#link("https://github.com/erlkonig-ai/faculties")[github.com/erlkonig-ai/faculties];
install from the documented sibling source cohort, or grab a
precompiled release tarball (every faculty plus the `trible`
CLI — no Rust toolchain needed; the `bootstrap` binary imports
the onboarding tour you are reading under your own pile signer).
...
== Mental model
  ...
  - One faculty = one cognitive verb:
    ...
    and so on.
  - Each durable domain uses one or more fixed, self-describing *collections*
    in the pile (`compass`, `wiki`, `files`, …). A descriptor fixes the member
    encoding and independent READ and WRITE policies. A faculty may read several and
...
The agent acts through shell commands and observes concrete output. A faculty
is the smallest possible "verb you can run from a shell that produces a durable
...
value by construction. Faculties therefore contain no reconciliation or
    mutable-head machinery. Operators activate exact collection handles explicitly;
    native transport repairs admitted records and their requested blob closure.
