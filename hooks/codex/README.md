...
- Print the report into the exec log, then pass it as one quoted `--message`
  argument to `codex queue --thread ID`, without an added preamble. No output
  is evaluated; event text and sender attribution are preserved. The report is
  forwarded tool data, not new user-authored instructions.
