...
impl FilesLive {
    fn refresh(dataset: DatasetView<'_>) -> Self {
        ...
        // Source path — a Handle<LongString>, dereferenced via the dataset reader on
        ...
        for (id, h) in path_rows {
            if let Some(row) = by_id.get_mut(&id) {
                row.source_path = read_text(dataset, h);
            }
        }
    ...
    pub fn render(&mut self, ctx: &mut CardCtx<'_>, dataset: DatasetView<'_>) {
        ...
        if let Some(root) = open_root {
            if let Some(live) = self.live.as_ref() {
                open_entity(dataset, &live.space, root);
            }
        }
    }
}
...
fn open_entity(dataset: DatasetView<'_>, space: &TribleSet, entity_id: Id) {
    let tmp_dir = std::env::temp_dir().join("faculties-files");
    if let Err(e) = std::fs::create_dir_all(&tmp_dir) {
        eprintln!("[files] mkdir {}: {e}", tmp_dir.display());
        return;
    }
    match extract_tree(dataset, space, entity_id, &tmp_dir, 0) {
        Ok(path) => {
...
fn extract_tree(
    ...
    for child in children {
        ...
        if let Err(e) = extract_tree(dataset, space, child, &dir, depth + 1) {
