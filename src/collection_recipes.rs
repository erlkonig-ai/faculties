...
fn succinctarchive_descriptor() -> KindDescriptor {
    KindDescriptor {
        ...
        epoch: ValidatorEpoch("raw-succinctarchive-union-v1/validator-v1"),
...
fn simplearchive_to_succinct_descriptor() -> DeriveDescriptor {
    DeriveDescriptor {
        source: simplearchive_kind(),
        target: succinctarchive_kind(),
        epoch: ValidatorEpoch("simplearchive-to-raw-succinctarchive/validator-v1"),
...
fn validate_succinctarchive_merge(
    ...
    let low = match SuccinctArchive::<OrderedUniverse>::try_from_blob(low) {
        Ok(archive) => archive,
        Err(error) => {
            return Ok(ValidationVerdict::Rejected(format!(
                "invalid succinct merge low input: {error}"
            )))
        }
    };
    let high = match SuccinctArchive::<OrderedUniverse>::try_from_blob(high) {
        Ok(archive) => archive,
        Err(error) => {
            return Ok(ValidationVerdict::Rejected(format!(
                "invalid succinct merge high input: {error}"
            )))
        }
    };
    let result = match SuccinctArchive::<OrderedUniverse>::try_from_blob(result) {
        Ok(archive) => archive,
        Err(error) => {
            return Ok(ValidationVerdict::Rejected(format!(
                "invalid succinct merge result: {error}"
            )))
        }
    };
...
fn validate_simplearchive_to_succinct(
    ...
    if source_rows.is_empty() {
        let canonical_empty =
            SuccinctArchive::<OrderedUniverse>::build_blob_pair(&TribleSet::new()).0;
        if target.bytes != canonical_empty.bytes {
            return Ok(ValidationVerdict::Rejected(
                "empty projection is not the canonical empty raw SuccinctArchive".to_owned(),
            ));
        }
    }
    let target = match SuccinctArchive::<OrderedUniverse>::try_from_blob(target) {
        Ok(archive) => archive,
        Err(error) => {
            return Ok(ValidationVerdict::Rejected(format!(
                "invalid raw SuccinctArchive derive output: {error}"
            )))
        }
    };
