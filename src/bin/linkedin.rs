...
struct Cli {
    ...
    key: Option<PathBuf>,
    /// Extrinsic Relations collection scope.
...
impl RelationsStorage<'_> {
    /// Read one immutable collection snapshot and validate its complete
    /// authored ontology before any import decision is made.
    ...
    fn publish(&self, view: &CollectionView, fragment: Fragment, description: &str) -> Result<()> {
...
fn build_lookup(view: &CollectionView) -> Result<Lookup> {
...
fn plan_import(view: &CollectionView, conns: &[Conn]) -> Result<IngestPlan> {
    let CanonicalInput {
        components,
        skipped,
...
fn ingest(storage: RelationsStorage<'_>, conns: &[Conn], dry_run: bool) -> Result<()> {
    ...
    let plan = plan_import(&view, conns)?;

    let committed = if dry_run || plan.fragment.facts().is_empty() {
        // Dry runs still validate the exact prospective union, including
        // staged text payloads. They differ only at publication.
        relations::validate_catalog_union(&view.reader, &view.facts, &plan.fragment)
            .context("preflight LinkedIn import")?;
        false
    } else {
        storage.publish(&view, plan.fragment, "linkedin: import connections")?;
        true
    };

    println!();
    println!("  new people:        {}", plan.created);
    println!(
        "  matched by email:  {}   (components with existing email evidence)",
        plan.matched_by_email
    );
    println!(
    ...
    println!(
        ...
        plan.prospective_collisions.len()
    );
    if plan.skipped > 0 {
        println!(
    ...
    if !plan.prospective_collisions.is_empty() {
        println!("\nProspective name collisions (derived, not persisted):");
        for (new_id, existing, name) in &plan.prospective_collisions {
...
fn describe(view: &CollectionView, id: Id) -> Result<String> {
...
fn derived_review_pairs(view: &CollectionView) -> Result<Vec<(Id, Id)>> {
...
fn cmd_resolve(
    ...
    storage.publish(&view, fragment, "linkedin: identity verdict")?;
...
mod tests {
    ...
    impl Fixture {
        ...
        fn publish(&self, fragment: Fragment) {
            let view = self.view();
            self.storage()
                .publish(&view, fragment, "test relations input")
                .unwrap();
        }
    }
    ...
    fn duplicate_import_is_a_no_op_and_url_spelling_is_canonical() {
        ...
        let first_commits = first.commits.len();
        ...
        assert_eq!(second.commits.len(), first_commits);
    }
    ...
    fn repeated_multi_key_import_does_not_depend_on_handle_order() {
        ...
        assert_eq!(second.commits.len(), commits);
    }
    ...
    fn equivalent_keys_preserve_existing_generic_values_byte_for_byte() {
        ...
        let head = relations::current_profile(&before.facts, person)
            .unwrap()
            .id;
        let commits = before.commits.len();
        ...
        assert_eq!(after.commits.len(), commits);
