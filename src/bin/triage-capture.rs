//! Minimal capture target for iterating on the triage widget.

use std::path::PathBuf;

use faculties::widgets::{SourceKey, StorageState, TriageViewer};
use GORBIE::notebook;
use GORBIE::prelude::*;
...
fn main(nb: &mut NotebookCtx) {
    ...
    nb.state("triage", TriageViewer::default(), move |ctx, panel| {
        ...
        let Some(view) = st.context().dataset(SourceKey::Triage) else {
            return;
        };
