...
const TOOLS: &[Tool] = &[
    ...
    Tool { name: "orient_poll", description: "One finite directed-news observation for an explicit persona. Defaults to non-consuming peek=true. peek=false records exact reported event IDs after output acceptance; unavailable selected payloads remain pending. No Habit scripts or server wait.", input_schema: r#"{"type":"object","properties":{"persona":{"type":"string"},"peek":{"type":"boolean","default":true}},"required":["persona"],"additionalProperties":false}"# },
